...
# ============================================================================
# RENTAL CONTRACT MODELS (Synced from POS)
# ============================================================================

class RentalContract(models.Model):
    """
    Rental contract synced from POS system.
    Tracks equipment rentals, deposits, and return status.
    """
    ...
    external_id = models.CharField(
        max_length=100, 
        db_index=True,
        help_text="POS rental contract ID"
    )
    ...
    # Contact info (may differ from customer record during rental)
    ...
    # Sync metadata
    external_source = models.CharField(max_length=20, default='POS')
    last_synced_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['tenant', 'expected_return_date']),
            models.Index(fields=['tenant_customer']),
        ]
        verbose_name = 'Rental Contract'
        verbose_name_plural = 'Rental Contracts'
    
    def __str__(self):
        return f"{self.contract_number} - {self.customer_name or 'Unknown'}"
    
    @property
    def is_overdue(self):
        """Check if rental is overdue"""
        if self.status in ['returned', 'closed', 'cancelled']:
            return False
...
class RentalContractItem(models.Model):
    """
    Individual items within a rental contract.
    Tracks products rented, quantities, and return status.
    """
    ...
    # Product details (denormalized from POS)
    ...
    def outstanding_quantity(self):
        """Calculate items not yet returned"""
        return self.quantity - self.returned_quantity - self.damaged_quantity - self.missing_quantity
    
    @property
    def is_fully_returned(self):
        """Check if all items are accounted for"""
        return self.outstanding_quantity == 0
